...
I've been playing around with OpenAI's function calling feature and wanted a simple framework to test my own function calls in a structured way. I wanted an easy-to-use setup where I could define tools and functions separately and have my chatbot intelligently decide when to call them.
I also decided to build everything around a container image, which should make it easy to spin everything up in any environment, while keeping the actual code outside of the container, removing the need to constantly having to rebuild the container during development. Here's how I built it.
